...
HOSTNAME=aqua1
ISO_FQN=/iso/ubuntu-18.04.3-server-amd64.iso
KS_FQN=ubuntu.ks
RAM=900
VCPUS=1
STORAGE=18
...
virt-install \
    --connect=qemu:///system \
    --name="${HOSTNAME}" \
    --disk="${VM_IMAGE_DIR}/${HOSTNAME}.qcow2,size=${STORAGE},bus=scsi,discard='unmap',format='qcow2'" \
    --ram="${RAM}" \
    --vcpus="${VCPUS}" \
    --autostart \
    --hvm \
